...
const PostItem: React.FC<PostItemProps> = ({ data = {}, userId }) => {
  ...
  const displayData =
    data.isCited === true && citedPost && citedPost.user ? citedPost : data;

  const { data: currentUser } = useCurrentUser();
  const { hasLiked, toggleLike } = useLike({ postId: displayData.id, userId });
  const { hasBookmarked, toggleBookmark } = useBookmark({
    postId: displayData.id,
    userId,
  });
  const { hasCited, toggleCitation } = useCitation({
    postId: displayData.id,
    userId,
  });
  ...
  const copyLinkToPost = useCallback(
    (ev: any) => {
      ev.stopPropagation();
  ...
  const goToCitationUser = useCallback(
    (ev: any) => {
      ev.stopPropagation();
    ...
    >
      {data.isCited === true && citedPost && citedPost.user && (
        <div className="mt-0flex flex-row flex-wrap">
          <span
            onClick={goToCitationUser}
            className="text-neutral-500 text-sm w-full flex flex-row gap-2 ml-10 hover:underline"
          >
        ...
        <div className="w-full">
          <div className="flex flex-row items-center gap-2 w-full">
            ...
            >
              <PostItemDots
                data={
                  data.isCited === true && citedPost ? citedPost : displayData
                }
                userId={displayData.user.id}
              />
            </div>
          </div>
          <div className="text-white mt-1 w-full">{displayData.body}</div>
          <div className="flex flex-row justify-between mt-3 gap-10">
            ...
            >
              <CiteIcon
                className={hasCited ? "text-blue-500 fill-blue-500" : ""}
                size={24}
              />
              <p>{citedLength}</p>
            </div>
            ...
            >
              <BsBookmarksFill
                className={hasBookmarked ? "text-blue-500 fill-blue-500" : ""}
                size={24}
              />
              <p>{bookmarkedLength}</p>
            </div>
            <HoverCard>
              <HoverCardTrigger>
                <div
